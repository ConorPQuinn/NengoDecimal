...
from nengo.utils.numpy import castDecimal, castFloat
...
class LIF(LIFRate):
    ...
    def step_math(self, dt, J, spiked, voltage, refractory_time):
        if True:
            dt = castDecimal(dt)
            J = castDecimal(J)
            spiked = castDecimal(spiked)
            voltage= castDecimal(voltage)
            refractory_time = castDecimal(refractory_time)
           
            tau_rc_decimal = castDecimal(self.tau_rc)
            min_volatge_decimal = castDecimal(self.min_voltage)
            tau_ref_decimal = castDecimal(self.tau_ref)
            amplitude_decimal = castDecimal(self.amplitude)
            ...
            temp = dt - refractory_time
            
            delta_t = (np.float64(temp)).clip(0, np.float64(dt))
    
            delta_t = castDecimal(delta_t)
            # update voltage using discretized lowpass filter
            # since v(t) = v(0) + (J - v(0))*(1 - exp(-t/tau)) assuming
            # J is constant over the interval [t, t + dt)
            
            voltage -= (J - voltage) * ((-delta_t / tau_rc_decimal).exp()-1)
    
            # determine which neurons spiked (set them to 1/dt, else 0)
            if voltage > 1:
                spiked[:] = (amplitude_decimal/ dt)
            else:
                spiked[:] = dc.Decimal(0)
            
            spiked_mask = voltage > 1
            #spiked[:] = spiked_mask * (amplitude_decimal/ dt)
            #edit here!!!
            # set v(0) = 1 and solve for t to compute the spike time
            temp =-(voltage[spiked_mask] - 1) / (J[spiked_mask] - 1)+1
            
            logtemp=np.asarray([p.ln() for p in temp])
            multiplytemp = np.asarray([p*tau_rc_decimal for p in logtemp])
            t_spike = dt + multiplytemp
    
            # set spiked voltages to zero, refractory times to tau_ref, and
            # rectify negative voltages to a floor of min_voltage
            voltage[voltage < min_volatge_decimal] = min_volatge_decimal
            voltage[spiked_mask] = 0
            refractory_time[spiked_mask] = tau_ref_decimal + t_spike
            
            refractory_time = castFloat(refractory_time)
            voltage =castFloat(voltage)
            dt = castFloat(dt)
            J = castFloat(J)
            spiked = castFloat(spiked)
