...
class LIF(LIFRate):
    ...
    def step_math(self, dt, J, spiked, voltage, refractory_time):
     ...
     # update voltage using accurate exponential integration scheme
            ...
            dV = -((-dt/tau_rc_decimal).exp()-1) * (J-voltage)
            print(dV)
            print ('v1',voltage)
            voltage += dV
            print ('v2',voltage)
            voltage[voltage < self.min_voltage] = castDecimal(self.min_voltage)
            print ('v3',voltage)
    ...
    #        voltage *= (1 - refractory_time / dt).clip(0, 1)
            ...
            temp = (1 - refractory_time / dt)
            print (temp)
            temp = castFloat(temp).clip(0,1)
            temp = castDecimal(temp)
...
#            print(voltage)
            ...
            voltage[spiked > 0] = 0
            refractory_time[spiked > 0] = tau_ref_decimal + spiketime
